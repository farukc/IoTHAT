...
namespace TurtaIoTHAT
{
    ...
    public class IOPortCapacitiveTouchEventArgs : EventArgs
    {
        private int ch;
        private bool state;

        // Constructor
        public IOPortCapacitiveTouchEventArgs(int ch, bool state)
        {
            this.ch = ch;
            this.state = state;
        }
    ...
    public delegate void IOPortCapacitiveTouchEventHandler(object sender, IOPortCapacitiveTouchEventArgs e);
    ...
    public class IOPort : IDisposable
    {
        ...
        // Interrupt Pin
        private static GpioPin capacitiveTouchInt;
        ...
        /// <summary>
        /// Initiates the IO Port function to use GPIO, analog input and capacitive touch functions.
        /// </summary>
        ...
        /// <param name="a1Cap">Set true for capacitive touch input, false for analog input.</param>
        /// <param name="a2Cap">Set true for capacitive touch input, false for analog input.</param>
        /// <param name="a3Cap">Set true for capacitive touch input, false for analog input.</param>
        /// <param name="a4Cap">Set true for capacitive touch input, false for analog input.</param>
        public IOPort(bool d1In, bool d2In, bool d3In, bool d4In, bool a1Cap, bool a2Cap, bool a3Cap, bool a4Cap)
        {
            ...
            capacitiveTouchInt = gpioController.OpenPin(27);
            ...
            capacitiveTouchInt.SetDriveMode(GpioPinDriveMode.InputPullDown);
            ...
            capacitiveTouchInt.DebounceTimeout = new TimeSpan(100000);
            ...
            switch (ch)
            {
                case 1:
                    if (multipleRead)
                    {
                        ...
                        for (int i = 0; i < 50; i++)
                            tempReadInt += ReadRegister_TwoBytes(0x20);

                        tempRead = Convert.ToUInt16(tempReadInt / 50);
                    }
                    else
                        tempRead = ReadRegister_TwoBytes(0x20);
                    break;
                ...
                case 2:
                    if (multipleRead)
                    {
                        ...
                        for (int i = 0; i < 50; i++)
                            tempReadInt += ReadRegister_TwoBytes(0x21);

                        tempRead = Convert.ToUInt16(tempReadInt / 50);
                    }
                    else
                        tempRead = ReadRegister_TwoBytes(0x21);
                    break;
                ...
                case 3:
                    if (multipleRead)
                    {
                        ...
                        for (int i = 0; i < 50; i++)
                            tempReadInt += ReadRegister_TwoBytes(0x22);

                        tempRead = Convert.ToUInt16(tempReadInt / 50);
                    }
                    else
                        tempRead = ReadRegister_TwoBytes(0x22);
                    break;
                ...
                case 4:
                    if (multipleRead)
                    {
                        ...
                        for (int i = 0; i < 50; i++)
                            tempReadInt += ReadRegister_TwoBytes(0x23);

                        tempRead = Convert.ToUInt16(tempReadInt / 50);
                    }
                    else
                        tempRead = ReadRegister_TwoBytes(0x23);
                    break;
        ...
        /// <summary>
        /// Notifies on capacitive touch state change.
        /// </summary>
        public event IOPortCapacitiveTouchEventHandler CapacitiveTouchStateChanged;
